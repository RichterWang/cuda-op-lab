...
void cublas_sgemm_row_major(
  ...
  // cuBLAS 为列主序；这里计算 C^T=B^T*A^T，等价于行主序 C=A*B。
  check_cublas(
...
int main() {
  ...
  try {
    ...
    check_cuda(cudaMemcpy(d_a, h_a.data(), sizeof(float) * h_a.size(),
                          cudaMemcpyHostToDevice),
               "copy a");
    check_cuda(cudaMemcpy(d_b, h_b.data(), sizeof(float) * h_b.size(),
                          cudaMemcpyHostToDevice),
               "copy b");
    ...
    check_cuda(cudaMemcpy(h_cublas.data(), d_c,
                          sizeof(float) * h_cublas.size(),
    ...
    for (size_t index = 0; index < h_cpu.size(); ++index) {
      const float abs_error = std::fabs(h_cpu[index] - h_cublas[index]);
    ...
    // added naive check
    check_cuda(cudaMemset(d_c, 0, sizeof(float) * h_naive.size()),
               "clear naive output");

    cuda_op_lab::sgemm::launch_sgemm_naive(
        d_a, d_b, d_c, m, n, k);

    check_cuda(cudaGetLastError(), "launch naive kernel");
    check_cuda(cudaDeviceSynchronize(), "wait naive kernel");

    check_cuda(cudaMemcpy(h_naive.data(), d_c,
                          sizeof(float) * h_naive.size(),
                          cudaMemcpyDeviceToHost),
               "copy naive output");
    ...
    for (size_t index = 0; index < h_cpu.size(); ++index) {
      ...
      naive_max_rel_error =
          std::max(naive_max_rel_error, abs_error / denominator);
    }
    ...
    return max_rel_error < 1.0e-4f && naive_max_rel_error < 1.0e-4f ? 0 : 1;
